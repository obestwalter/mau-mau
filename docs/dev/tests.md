# Simple testing with [py.test](http://pytest.org)

**Note**: test modules are here: [tests/](https://github.com/obestwalter/mau-mau/blob/master/mau_mau/tests/)
...
This test failed not because the code is broken, but because we had a wrong assertion about the program. If you pass a list, when you create the `Stock` you should not expect is to be empty afterwards.
